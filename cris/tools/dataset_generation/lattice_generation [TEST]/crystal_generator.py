...
class CrystalLattice:
    ...
    LATTICE_TYPES = {
        ...
        # Кубическая сингония
        'cubic_primitive': {
            'name': 'Кубическая примитивная (SC)',
            'syngony': 'cubic',
            'centering': 'P'
        },
        'cubic_body': {
            'name': 'Кубическая объемно-центрированная (BCC)',
            'syngony': 'cubic',
            'centering': 'I'
        },
        'cubic_face': {
            'name': 'Кубическая гране-центрированная (FCC)',
            'syngony': 'cubic',
            'centering': 'F'
        },
    }
    ...
    def get_basis_positions(self) -> List[np.ndarray]:
        """Возвращает позиции ионов в элементарной ячейке (в долях)"""
    ...
    def generate_lattice(self, nx: int, ny: int, nz: int,
        ...
        """
        Генерирует координаты ионов решетки
        ...
        Returns:
            Массив координат ионов формы (N, 3)
        """
            ...
            for j in range(ny):
                for k in range(nz):
                    ...
                    for basis_pos in basis_positions:
                        # Проверка на вакансию (случайное удаление ионов)
                        if vacancy_prob > 0 and np.random.random() < vacancy_prob:
                            continue  # Пропускаем этот ион (создаем вакансию)
    ...
    for i in range(nx):
        for j in range(ny):
            for k in range(nz_start, nz_end):
                ...
                for basis_pos in basis_positions:
                    # Проверка на вакансию (случайное удаление иона)
                    if vacancy_prob > 0 and np.random.random() < vacancy_prob:
                        continue  # Пропускаем этот ион (создаем вакансию)
...
def save_xyz(filename: str, positions: np.ndarray, element: str = 'C'):
    """
